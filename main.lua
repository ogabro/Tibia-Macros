...
-- Macro for Challenge Spell"

macro(500, "Auto Challenge",  function()
...
--Knight Buff spell
local BuffSpell = 'Utito Gran Tempo'

macro(1, "Knight Buff", function()
    if g_game.isAttacking() and not isInProtectionZone() then
        if not modules.game_cooldown.isGroupCooldownIconActive(group) and not modules.game_cooldown.isCooldownIconActive(121) then
            say('Utito Gran Tempo')
        end
      end
end)

local mWallBehindEnabled = false

-- Function to toggle "MWall Behind" macro
local function toggleMWallBehind()
    mWallBehindEnabled = not mWallBehindEnabled
    if mWallBehindEnabled then
        print("MWall Behind macro is now active")
    else
        print("MWall Behind macro is now inactive")
    end
end

macro(1000, "MWall Behind", toggleMWallBehind)

onPlayerPositionChange(function(newPos, oldPos)
    if mWallBehindEnabled and oldPos.z == posz() then
